...
for prefix in ["harmony_", "old_pca_"]: # "new_pca_",
    ...
    # adata.obsm[f'X_{prefix}pool_sub_umap'] = np.empty((adata.n_obs, 2), dtype=np.float64)
    ...
    adata.obsm[f'X_{prefix}sample_sub_umap'] = np.empty((adata.n_obs, 2), dtype=np.float64)
